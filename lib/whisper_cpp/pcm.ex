defmodule WhisperCpp.Pcm do
  ...
  `WhisperCpp.Wav.read_file/1` and then runs many short transcribe calls
  over per-turn slices. `slice/4` does the byte math (4 bytes per sample
  at `sample_rate` samples/second) and bounds-checks against the buffer
  size.
  """
  ...
  def slice(_samples, _sample_rate, _start_s, _duration_s) do
    {:error, Error.new(:invalid_request, "invalid arguments to WhisperCpp.Pcm.slice/4")}
  end
  ...
  def duration_s(samples, sample_rate)
    ...
    div(byte_size(samples), @bytes_per_sample) / sample_rate
  end
